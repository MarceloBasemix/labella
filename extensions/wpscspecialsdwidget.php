...
<p>
	<input type="checkbox" class="checkbox"
		...
		<?php echo $show_description ? 'checked="checked"' : ""; ?>> <label
		for="<?php echo $this->get_field_id( 'show_description' ); ?>"><?php _e( 'Show Description', 'wpsc' ); ?></label><br />
	<input type="checkbox" class="checkbox"
		id="<?php echo $this->get_field_id( 'show_thumbnails' ); ?>"
		name="<?php echo $this->get_field_name( 'show_thumbnails' ); ?>"
		<?php echo $show_thumbnails ? 'checked="checked"' : ""; ?>> <label
		for="<?php echo $this->get_field_id( 'show_thumbnails' ); ?>"><?php _e( 'Show Thumbnails', 'wpsc' ); ?></label>
</p>
<?php
	}
}
...
<div class="grid no-border" id="products">
	...
	if (count ( $special_products ) > 0) {
		list ( $wp_query, $special_products ) = array (
				$special_products,
				$wp_query 
		...
		while ( wpsc_have_products () ) :
			...
			rel="<?php echo str_replace(array(" ", '"',"'", '&quot;','&#039;'), array("_", "", "", "",''), wpsc_the_product_title()); ?>"
			href="<?php echo wpsc_the_product_permalink(); ?>"> <img
				class="product_image"
				...
				<?php else: ?>
							<a href="<?php echo wpsc_the_product_permalink(); ?>"> <img
				class="no-image"
				id="product_image_<?php echo wpsc_the_product_id(); ?>"
				alt="No Image" title="<?php echo wpsc_the_product_title(); ?>"
				src="<?php echo WPSC_URL; ?>/wpsc-theme/wpsc-images/noimage.png"
				width="<?php esc_attr_e( get_option('product_image_width') ); ?>"
				height="<?php esc_attr_e( get_option('product_image_height') ); ?>" />
		</a>
				<?php endif; ?>
			...
			<h3 class="product-name">
				<a class="wpsc_product_title"
					href="<?php echo wpsc_product_url( wpsc_the_product_id(), false ); ?>"><?php echo wpsc_the_product_title(); ?></a>
			</h3>
			<br /> <span id="special_product_price_<?php echo wpsc_the_product_id(); ?>"> <!-- price display -->
				<?php
...
if (wpsc_have_variation_groups ()) :
					...
					$preco_normal = $wpdb->get_results ( "SELECT * FROM $wpdb->postmeta WHERE post_id = $id_produto AND meta_key = '_wpsc_price'" );
					...
					if (count ( $precos ) == 1) :
						...
						if ($id_produto_variacao) {
							...
							$precos = $wpdb->get_results ( "SELECT * FROM $wpdb->postmeta WHERE post_id = $id_produto_variacao AND meta_key = '_wpsc_price'" );
						}
							
							if($precos[0]->meta_value > 0){
	
								echo '
										<span class="oldprice" id="old_product_price_'. wpsc_the_product_id() .'"
										style="margin-left: 10px !important; font-size: 12px !important; color: #424449;">'. wpsc_product_normal_price( $preco_normal[0]->meta_value ) . '</span><br />
			 						';
							}else{
							 	$precos[0]->meta_value =  $preco_normal[0]->meta_value;
							}
					...
					foreach ( $precos as $values ) {
						$price = $values->meta_value;
				...
				<div class="price-box">
					...
					<span class="oldprice" id="old_product_price_<?php echo wpsc_the_product_id(); ?>"
						style="margin-left: 70px !important; font-size: 12px !important;"><?php echo wpsc_product_normal_price(); ?></span><br />
					<?php endif; ?>
